...
class ESMMAdapter(TrainingAdapter):
    ...
    def adapt_output(self, model: BaseMultitaskModel, model_output):
        ctr, cvr = model_output.chunk(2, dim=1)
        ctcvr = ctr * cvr
...
class ESMM(BaseMultitaskModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        training_mode = kwargs.get("training_mode", "pointwise")
        if training_mode != "pointwise":
            raise ValueError(f"ESMM only supports training_mode='pointwise', got '{training_mode}'.")
