...
def run_model_batch(model, data, batch_size: int | None = None, require_labels: bool = False):
    effective_batch_size = batch_size or len(next(iter(data.values())))
...
class TestInBatchTraining:
    ...
    def test_inbatch_cross_entropy_loss_is_supported(
        ...
        batch_size = 5
        data = self.build_inbatch_data(num_samples=batch_size)
        y_pred, _ = run_model_batch(model, data, batch_size=batch_size, require_labels=False)
...
class TestSamplingModeValidation:
    def test_explicit_candidate_lists_require_list_axis(self, device):
        ...
        flat_data = {
            "user_id": np.random.randint(1, 100, size=4, dtype=np.int64),
            "item_id": np.random.randint(1, 100, size=4, dtype=np.int64),
        }
    ...
    def test_explicit_candidate_lists_accept_list_axis(self, device):
        ...
        list_size = 4
        explicit_data = {
            "user_id": np.random.randint(1, 100, size=batch_size, dtype=np.int64),
            "item_id": np.random.randint(1, 100, size=(batch_size, list_size), dtype=np.int64),
        }

        with torch.no_grad():
            output, _ = run_model_batch(model, explicit_data, batch_size=batch_size, require_labels=False)
